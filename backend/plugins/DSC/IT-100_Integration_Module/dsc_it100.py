...
def verify_checksum(raw: str) -> bool:
    """Verify the checksum of a raw packet string (without CR/LF)."""
...
def _parse_data(command: str, data: str) -> dict:
    ...
    # --- Zone commands (zone only) ---
    if command in (CMD_ZONE_OPEN, CMD_ZONE_RESTORED,
                   CMD_ZONE_FAULT, CMD_ZONE_FAULT_RESTORE):
        p['zone'] = int(data) if data.isdigit() else data

    # --- Zone + partition commands ---
    elif command in (CMD_ZONE_ALARM, CMD_ZONE_ALARM_RESTORE,
                     CMD_ZONE_TAMPER, CMD_ZONE_TAMPER_RESTORE):
        if len(data) >= 4:
            p['partition'] = int(data[0])
            p['zone']      = int(data[1:])

    # --- Single-partition commands ---
    elif command in (CMD_PARTITION_READY, CMD_PARTITION_NOT_READY,
                     CMD_PARTITION_READY_FORCE, CMD_PARTITION_IN_ALARM,
                     CMD_PARTITION_DISARMED, CMD_EXIT_DELAY,
                     CMD_ENTRY_DELAY, CMD_KEYPAD_LOCKOUT,
                     CMD_KEYPAD_BLANKING, CMD_COMMAND_OUTPUT_PROGRESS,
                     CMD_INVALID_ACCESS_CODE, CMD_FUNCTION_NOT_AVAILABLE,
                     CMD_FAIL_TO_ARM, CMD_PARTITION_BUSY,
                     CMD_SPECIAL_CLOSING, CMD_PARTIAL_CLOSING,
                     CMD_SPECIAL_OPENING, CMD_TROUBLE_LED_ON,
                     CMD_TROUBLE_LED_OFF):
        p['partition'] = int(data[0]) if data else None

    # --- Partition Armed (mode included) ---
    elif command == CMD_PARTITION_ARMED:
        if len(data) >= 2:
            p['partition'] = int(data[0])
            p['mode']      = ARM_MODES.get(data[1], data[1])

    # --- User closing/opening (partition + user code) ---
    elif command in (CMD_USER_CLOSING, CMD_USER_OPENING):
        if len(data) >= 5:
            p['partition'] = int(data[0])
            p['user']      = int(data[1:])

    # --- Code required ---
    elif command == CMD_CODE_REQUIRED:
        if len(data) >= 2:
            p['partition']   = int(data[0])
            p['code_length'] = int(data[1])

    # --- System error ---
    elif command == CMD_SYSTEM_ERROR:
        p['error_code']        = data
        p['error_description'] = ERROR_CODES.get(data, 'Unknown error')

    # --- Command acknowledge ---
    elif command == CMD_COMMAND_ACK:
        p['acknowledged_command'] = data

    # --- Software version ---
    elif command == CMD_SOFTWARE_VERSION:
        if len(data) >= 4:
            p['version']     = data[0:2]
            p['sub_version'] = data[2:4]

    # --- LED status ---
    elif command == CMD_LED_STATUS:
        led_names = {
        ...
        if len(data) >= 2:
            p['led']   = led_names.get(data[0], data[0])
            p['state'] = state_names.get(data[1], data[1])

    # --- LCD update ---
    elif command == CMD_LCD_UPDATE:
        if len(data) >= 6:
            p['line']       = int(data[0])
            p['column']     = int(data[1:3])
            p['char_count'] = int(data[3:5])
            p['text']       = data[5:]

    # --- Baud rate set ---
    elif command == CMD_BAUD_RATE_SET:
        rates = {'0': 9600, '1': 19200, '2': 38400, '3': 57600, '4': 115200}
        p['baud_rate'] = rates.get(data, data)

    # --- Thermostat / temperature ---
    elif command in (CMD_INDOOR_TEMP, CMD_OUTDOOR_TEMP):
        if len(data) >= 4:
            p['thermostat']   = int(data[0:2])
            p['temperature']  = int(data[2:])

    elif command == CMD_THERMOSTAT_SET_POINTS:
        if len(data) >= 8:
            p['thermostat']  = int(data[0:2])
            p['cool_set']    = int(data[2:5])
            p['heat_set']    = int(data[5:8])
...
class IT100:
    ...
    def __init__(self, port: str, baud: int = 9600, timeout: float = 1.0):
        ...
        # Pending code-required callbacks: partition -> callable
        self._code_callbacks: dict[int, Callable] = {}
    ...
    def on(self, event: str, callback: Callable):
        ...
        The callback receives a single dict argument with parsed fields.

        Example:
            panel.on('zone_open', lambda d: print(d['zone']))
            panel.on('609',       lambda d: print(d['zone']))
        """
        code = EVENTS.get(event, event)   # resolve alias → code
        self._listeners.setdefault(code, []).append(callback)

    def _emit(self, packet: dict):
        """Fire all listeners registered for this command code."""
        code      = packet['command']
        listeners = self._listeners.get(code, [])
        for cb in listeners:
            try:
                cb(packet['parsed'])
            except Exception as exc:
                logger.error(f'Listener error for {code}: {exc}')
    ...
    def _handle_raw(self, raw: str):
        ...
        logger.debug(f'RX {raw!r}  →  {packet["parsed"]}')
        ...
        if packet['command'] == CMD_CODE_REQUIRED:
            ...
            if partition and partition in self._code_callbacks:
                cb = self._code_callbacks.pop(partition)
                cb()
    ...
    def bypass_zone(self, zone: int, code: Optional[str] = None):
        ...
        zone_str = f'{zone:02d}'
        self.key_press(zone_str[0])
        self.key_press(zone_str[1])
    ...
    def arm_with_auto_code(self, partition: int, code: str,
        ...
        self._code_callbacks[partition] = _send_code

        if mode == 'away':
            self.arm_away(partition)
        elif mode == 'stay':
            self.arm_stay(partition)
        elif mode == 'no_delay':
            self.arm_no_entry_delay(partition)
        else:
